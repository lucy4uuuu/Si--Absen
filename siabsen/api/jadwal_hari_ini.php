...
if ($user['role'] === 'dosen') {
    // Dosen: jadwal yang dia ampu hari ini
    $stmt = $db->prepare("
    ...
    $prodiMhs = $prodiStmt->fetchColumn();

    // Jadwal hari ini sesuai prodi DAN terdaftar di enrollment
    $stmt = $db->prepare("
        SELECT j.* FROM jadwal j
        INNER JOIN enrollment e ON e.jadwal_id = j.id AND e.nim = ?
        WHERE j.hari = ? AND (j.prodi = ? OR j.prodi IS NULL OR j.prodi = '')
        ORDER BY j.mulai
    ");
    $stmt->execute([$user['id'], $hariIni, $prodiMhs]);

} else {
    // Admin: semua jadwal hari ini
    $stmt = $db->prepare("SELECT j.* FROM jadwal j WHERE j.hari = ? ORDER BY j.mulai");
    $stmt->execute([$hariIni]);
}

$rows = $stmt->fetchAll();

// Absensi hari ini milik user ini
$type   = $user['role'] === 'dosen' ? 'dosen' : 'mahasiswa';
$absStmt = $db->prepare("
    SELECT mk, status, checkin, checkout
    FROM absensi WHERE user_id = ? AND type = ? AND tanggal = ?
");
